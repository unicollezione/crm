module Trello
  # class Trello::OrderWebhookService
  #
  ...
  class OrderWebhookService
    ...
    # @param [params] Trello webhook params from controller
    def initialize(params:)
      @params = params
      @status = :error
      # since Trello has same id for different cards we have to keep this workaround
      @order = Order
               .order(created_at: :desc)
               .find_by(trello_card_id: params[:path])
    end

    def call
      set_order_details

      @status = :ok if order.save
    ...
    def set_order_details
      order_aasm_state if update_card_list?
      order_desc if trello_desc?
      build_new_order_event
    end

    def build_new_order_event
      order.order_events.build(event_source: 'trello_notification', body: trello_webhook_payload)
    end
    ...
    def desc
      params.dig(:trello, :action, :data, :card, :desc)
    end

    def order_aasm_state
      order.aasm_state = list_after
    end

    def update_card_list?
      action_type == 'updateCard' && list_after.present?
    end
    ...
    # @return [Hash] trello webhook payload
    # @example
    #  {
    #    action_type: 'updateCard',
    #    card_name: '#140',
    #    creator: 'notification',
    #    list_before: 'В Работе',
    #    list_after: 'очередь'
    #    payload: {
    #     ...
    #    }
    ...
    # @return [String] webhook action type (createCard, updateCard, addAttachmentToCard)
    def webhook_action
    ...
    # @return [String] 'alxekb'
    def creator
      params.dig(:trello, :action, :memberCreator, :username)
    end

    # @return [String] 'В Работе'
    def list_after
      params.dig(:trello, :action, :data, :listAfter, :name).presence ||
        params.dig(:trello, :action, :data, :list, :name).presence
    end

    # @return [String] 'очередь'
    def list_before
      params.dig(:trello, :action, :data, :listBefore, :name)

      params.dig('trello', 'action', 'data', 'display', 'entities', 'listAfter', 'text')
    end
  end
end
